      ...
      switch (data.command) {
        ...
        case 'test':
          await vscode.window.withProgress(
            ...
            async () => {
              const res = await DataProvider.getInstance().testConnection(data.connectionId);
              if (res.valid) {
          ...
          break;
        case 'toggleActive':
          const ok = await DataProvider.getInstance().toggleConnection(data.connectionId, data.nextActive);
          if (ok) {
